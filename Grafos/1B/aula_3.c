...
int if_girl(int value, int *arry, int qtd);
...
int main(){
    ...
    array_girls = (int *)calloc(qtd_girls, sizeof(int));
...
/*
    if((qtd_arestas == (qtd_vertices-1)) && count == 1){
        printf("\nE uma arvore!!!\n");
    }
*/

    for(int i = 2;i <= qtd_vertices;i++){
        if((near_girl == 0 || menor > vertices[i].distancia) && if_girl(i,array_girls,qtd_girls)){
